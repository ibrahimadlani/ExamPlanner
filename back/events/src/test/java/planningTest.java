...
public class planningTest {
    ...
    void setUp(){
        ...
        this.planning = new Planning(LocalDateTime.of(2022,12,05, 0,0),LocalDateTime.of(2022,12,12,0,0), list_indisponibilites, 8,0, 17,0,toutes_matieres);
    ...
    public void generateOK(){
        ...
        assertEquals(this.planning.getMatieres().size(), this.planning.getExamens().size());
    }

    @Test
    public void testGenerateOverflow(){
        System.out.println(this.planning);
        // Set les durées d'examens trop grandes pour ne pas pouvoir remplir l'emploi du temps sans déborder
        for (Matiere m: this.planning.getMatieres()
             ) {
            m.setDuree_examen(m.getDuree_examen()*2);
        }

        // Assert
        this.planning.generate();
        assertNotEquals(planning.getMatieres().size(), planning.getExamens().size());
    }
}
